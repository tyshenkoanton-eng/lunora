...
## Производные строки не хранятся копией 🔒

Всё, что можно вычислить из уже записанного, вычисляется, а не дублируется вторым списком.

Правило появилось из двух ошибок прототипа. Карточки школ на экранах тем и тексты самих школ писались отдельно: после переписывания школ карточки обещали одно, а по ссылке открывалось другое. Подписи для шеринга жили третьим списком, и девять тем из одиннадцати остались вовсе без подписи.

Что из этого следует для реализации:

| Строка | Откуда берётся | Не хранить |
|---|---|---|
| Подпись при шеринге темы | название темы + число школ, которые её считают (по матрице) | отдельный словарь подписей |
| Короткая выжимка школы на экране темы | поле `summary` той же ячейки, см. ниже | вручную написанная копия |
| Счётчики тем и разделов | длина матрицы | число в тексте |

🔒 Если строка выглядит как пересказ другой строки, она должна вычисляться из неё. Копия расходится с оригиналом при первой же правке, и расхождение обнаруживает не автор, а читатель.

### Резюме ячейки — поле, а не обрезка

Вычислять можно не всё. Выжимку школы для экрана темы прототип сейчас берёт обрезкой первого абзаца ячейки, и это работает, но даёт текст суше нужного: в ячейке уместно начать с механики («Область собственных средств планет не содержит»), а на уровне темы читателю нужна фраза о нём самом.

Поэтому резюме становится отдельным блоком рендера, а не обрезкой готового текста. Живёт оно там же, где остальной текст ячейки, — в `NarrativeRender`, потому что `CellBundle` текста не содержит по определению:

```
NarrativeRender:
  summary:                  # показывается на экране темы, не внутри ячейки
    text                    # одно-два предложения о человеке, не о методе
    claim_refs[]            # те же проверки происхождения, что и у прочих предложений
```

Резюме проходит ту же проверку смысловой точности, что и основной текст: оно не новый вывод, а сжатие уже подтверждённых claim'ов этой ячейки.

Правила блока:

| Требование | Почему |
|---|---|
| Говорит о человеке, а не о расчёте | Экран темы читают до того, как решили углубляться |
| Читается вне контекста заголовка | Карточка стоит отдельно; «Как к данности» без заголовка бессмысленно |
| Не длиннее двух предложений | Карточек на экране четыре, каждая соревнуется за внимание |
| Пишется один раз вместе с ячейкой | Источник остаётся один, правило выше не нарушается |

🔒 `summary` обязателен для каждой ячейки со статусом AVAILABLE. Пустое поле — дефект сборки, а не повод показать обрезку: обрезка допустима только как аварийный запасной вариант.

Проверка при сборке: резюме не начинается с зависимого оборота («Через», «Как», «Это»), не заканчивается многоточием и не повторяет дословно первый абзац.
